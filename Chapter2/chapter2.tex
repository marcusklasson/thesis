...
The variational autoencoder (VAE) is a popular deep generative models that uses approximate Bayesian inference in an efficient manner that scales well to large datasets. It originates from latent variable models, where models are denoted as the joint distribution $p_{\boldsymbol{\theta}}(\mathbf{x}, \mathbf{z}) = p_{\boldsymbol{\theta}}(\mathbf{x} | \mathbf{z})p(\mathbf{z})$ with a likelihood $p_{\boldsymbol{\theta}}(\mathbf{x} | \mathbf{z})$ and a prior $p(\mathbf{z})$. In VAEs, the likelihood is parameterized by a neural network with parameters $\boldsymbol{\theta}$ that transforms the latent variable $\mathbf{z}$ to the data $\mathbf{x}$. The distribution over $\mathbf{x}$ depends on type of data that we are generating, e.g. if the data is binary then the likelihood follows a Bernoulli distribution. Thus, the likelihood is referred to the generative network or decoder. The prior distribution is typically given by $p(\mathbf{z}) = \mathcal{N}(\mathbf{z} | \boldsymbol{0}, \mathbf{I})$ when $\mathbf{z}$ is a continuous latent variable as in the standard VAE. 

%We wish to find likely values of $\mathbf{z}$ that have generated $\mathbf{x}$, which is referred to as the inference task. by applying Bayes' rule 

%$p_{\boldsymbol{\theta}}(\mathbf{x} | \mathbf{z}) = f(\mathbf{x}; \mathbf{z}, \boldsymbol{\theta})$ is a suitable likelihood function parameterized by $\boldsymbol{\theta}$ and $p(\mathbf{z}) = \mathcal{N(\mathbf{z} | \boldsymbol{0}, \mathbf{I})}$ is the prior distribution over the latent variables. In VAEs, the likelihood function is a neural network where $\boldsymbol{\theta}$ represents the weights and biases. 

The goal now is to sample values of $\mathbf{z}$ that are likely to have produced our data $\mathbf{x}$. Ideally, we want to compute the true posterior distribution $p_{\boldsymbol{\theta}}(\mathbf{z} | \mathbf{x})$ by applying Bayes' rule, which unfortunately is intractable since $p_{\boldsymbol{\theta}}(\mathbf{x}) = \int p_{\boldsymbol{\theta}}(\mathbf{x}, \mathbf{z}) \, d\mathbf{z}$ is hard to compute due to the many settings of $\mathbf{z}$ we need to evaluate. We will therefore rely on variational inference techniques and try to approximate the true posterior distribution with a distribution $q_{\boldsymbol{\phi}}(\mathbf{z} | \mathbf{x})$ parameterized by $\boldsymbol{\phi}$. Commonly, this approximate posterior is a Gaussian distribution given by $q_{\boldsymbol{\phi}}(\mathbf{z} | \mathbf{x}) = \mathcal{N(\mathbf{z} | \boldsymbol{\mu}_{\boldsymbol{\phi}}(\mathbf{x}), \text{diag}(\boldsymbol{\sigma}_{\boldsymbol{\phi}}(\mathbf{x})))}$, where $\boldsymbol{\mu}_{\boldsymbol{\phi}}(\mathbf{x})$ and $\boldsymbol{\sigma}_{\boldsymbol{\phi}}(\mathbf{x})$ are the outputs from a neural network that takes $\mathbf{x}$ as input. This neural network is referred to as the inference network or encoder. 

The overall goal with generative models is that we want to maximize the mariginal log-likelihood $\log p_{\boldsymbol{\theta}}(\mathbf{x})$. However, evaluating $p_{\boldsymbol{\theta}}(\mathbf{x})$ was intractable as described above, but we can optimize a lower bound on $\log p_{\boldsymbol{\theta}}(\mathbf{x})$ referred to as the evidence lower bound (ELBO). The tractable optimization objective for the VAE is then given by the right-hand side of
\begin{equation}\label{eq:elbo}
	\log p_{\boldsymbol{\theta}}(\mathbf{x}) \geq \mathbb{E}_{q_{\boldsymbol{\phi}}(\mathbf{z} | \mathbf{x})}[\log p_{\boldsymbol{\theta}}(\mathbf{x} | \mathbf{z})] - D_{\text{KL}}[ q_{\boldsymbol{\phi}}(\mathbf{z} | \mathbf{x}) | p(\mathbf{z})] %= -\mathcal{J}(\mathbf{x})
	,
\end{equation}
which is derived from the Kullback-Leibler divergence between the approximate and true posterior distributions, $D_{\text{KL}}[ q_{\boldsymbol{\phi}}(\mathbf{z} | \mathbf{x}) | p_{\boldsymbol{\theta}}(\mathbf{z} | \mathbf{x})]$. When the ELBO in Equation \ref{eq:elbo} has been maximized, we can use the projected features $\mathbf{z}$ from the inference network $q_{\boldsymbol{\phi}}(\mathbf{z} | \mathbf{x})$ to train any type of classifier that predicts the class labels $y$. The classification is thus performed in a two-stage setting: (i) Learn feature mebeddings $\mathbf{z}$ of the data $\mathbf{x}$ and (ii) train a classifier on the pairs $(\mathbf{z}_i, y_i)$. 
