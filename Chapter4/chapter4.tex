...
The gap we see is that most replay methods assume that time is ignored in most works on CL, which is important in human learning. Furthermore, the memory storage is often assumed to be limited which is rarely the case in real-world applications for machine learning.

In Paper \ref{paperC}, we propose a new CL setting where memory storage is unlimited while the budget for mitigating catastrophic forgetting is limited. We then demonstrate the importance of time of replay by proposing replay scheduling where we select which tasks to replay at different times. In Paper \ref{paperD}, we present a framework based on reinforcement learning (RL)~\cite{sutton2018reinforcement} for learning replay scheduling policies that can generalize to new CL scenarios without added computational cost. 
...
\begin{figure}[t]
	...
	\caption{Illustration of catastrophic forgetting in a continual learning scenario. \MK{Should add thicker borders on the images and the classifier boxes!} }
