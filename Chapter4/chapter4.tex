...
In this section, we introduce the RL-based framework for learning replay scheduling policies that was proposed in Paper \ref{paperD}. 
In Paper \ref{paperD}, we aim to enable replay scheduling in real-world CL scenarios by learning policies that can be transferred to new CL environments to mitigate catastrophic forgetting. We propose an RL-based framework for policy learning and show that the policies can generalize to new task orders and datasets without additional computational cost in the test environments. 
