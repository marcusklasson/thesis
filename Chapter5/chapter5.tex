...
In recent years, there has been a growing interest for federated learning~\cite{konevcny2015federated, mcmahan2017communication, li2020federated} in the machine learning community. 
...
%Federated learning involves leveraging the storage and computational capabilities of remote devices, e.g., mobile phones, for training local models while keeping the data on the device to preserve privacy. 

\vspace{-3mm}
\paragraph{Example Scenario.} Here, we give an example application of federated learning in the context of assistive vision. Alice and Bob are using an assistive vision app installed on their mobile phones for recognizing groceries in a supermarket. 
%We give an example scenario in the context of assistive vision where Alice and Bob are grocery shopping in a supermarket using an assistive vision app installed on their personal smartphones. 
Today, 
...
%She captures several frames of the milk package by recording a video with the camera that is processed in the app. 
However, the app responds that this milk package is an unknown item.% since this type of milk brand is new in the supermarket. 
...
%Alice asks Charles, the store clerk, what item she is holding, gets help with updating her app with the new milk class from Charles, and purchases the milk. 
The next day, Bob does his grocery shopping in the same supermarket as Alice. 
Bob notices a new milk package he has never tried %Bob is curious about a new milk package he has never tried %seen 
...
tries to recognize this kind of milk package with his app. %takes photos of this kind of milk package with the app. 
...
%From the perspective of this thesis, it would be valuable to study the statistical heterogeneity as the generated data can vary immensely across different users in terms of sample frequency, data quality, and context of where the data has been recorded. Here, it may be important to consider issues regarding fairness in addition to accuracy, since the learned model may become biased towards devices with large amounts of data or to commonly occurring groups of devices.
...
An example of close collaboration between VI people and technical researchers is the collection of the recent ORBIT dataset~\cite{massiceti2021orbit, theodorou2021disability}. Here, the researchers took a \textit{disability-first approach} to produce a good quality dataset to benchmark computer vision methods in real-world object recognition scenarios for VI people. The data collection was only performed by blind/low-vision participants and the researchers were engaging continuously with the participants to support them in the process. Potentially, the disability-first approach can make VI people aware of technical challenges with current computer vision method, as well as bring insights to researchers on how to build more robust object recognizers to satisfy the needs of the target user. 
%Even if the combination is evidently a good fit, many of the mentioned works stress that researchers should keep VI people involved throughout the process to ensure that the developed technology is requested and, in fact, useful for blind/low-vision people. An example of close collaboration between the two communities is the collection of the recent ORBIT dataset~\cite{massiceti2021orbit, theodorou2021disability} where the data collection was performed by blind/low-vision participants. They take on a \textit{disability-first approach} in the process to produce good quality dataset that should primarily serve the VI community by continuously engaging with and supporting the participants to balance the usefulness versus the effort of data to be collected~\cite{theodorou2021disability}. Involving VI users in the research process should provide benefits on for both the VI users as well as the vision community by bringing insights on how to build more robust image recognizers. 

\MK{I think that this paragraph could be dedicated to fairness and privacy. Move the usefulness sentence to paragraph above.}
The research and development of assistive vision technologies must consider aspects on usefulness, fairness, and privacy of the users. Regarding usefulness, computer vision researchers need to investigate what type of assistance VI people want from computer vision, and discuss with other technical experts in, e.g., human-computer interaction and user experience designers, on how to build functionality to satisfy the needs.  
To establish fair computer vision systems, we also need to tackle challenges around different biases in object recognizers that can harm and disadvantage users based on their gender~\cite{bolukbasi2016man, buolamwini2018gender, burns2018women}, geographical location~\cite{de2019does}, and co-occurrences between objects and surrounding contexts~\cite{singh2020don}. 


VI people want to 

it is important that computer vision researchers engage with VI people as well as other technical experts in, e.g., human-computer interaction and user experience designers, to understand 

 will be important 

to engage with VI people and people from technical disciplines, such as human-computer interaction and user experience designers, to understand what VI people want their app to assist them with and how to build functionality to achieve those goals. 

Aspects on usefulness, fairness, and privacy needs to be considered when developing assistive vision technologies. Regarding usefulness, it will be important to engage with VI people and people from technical disciplines, such as human-computer interaction and user experience designers, to understand what VI people want their app to assist them with and how to build functionality to achieve those goals. 
For computer vision researchers, we have seen that the models suffer from biases that can disadvantage users based on their gender~\cite{bolukbasi2016man, buolamwini2018gender, burns2018women}, geographical location~\cite{de2019does}, and co-occurrences between objects and surrounding contexts~\cite{singh2020don}. 
